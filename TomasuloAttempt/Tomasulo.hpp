//
// Created by Snoopy on 2021/7/11.
//
...
const int fetch_queue_size = 32, res_size = 32, rob_size = 64, reg_file_size = 32;

bool flag_fetch_queue_to_issue = false;

unsigned inst_fetch_queue_to_issue;

struct Fetch_Queue {
    struct node {
        unsigned inst, pc;
    } queue[fetch_queue_size + 1];
    int head, tail, length;
    Fetch_Queue() : head(1), tail(1), length(1){}
    void push(unsigned _inst, unsigned _pc) {
        queue[tail].inst = _inst, queue[tail].pc = _pc;
        tail = tail % fetch_queue_size + 1;
        ++length;
    }
    void pop() {
        head = head % fetch_queue_size + 1;
        --length;
    }
    node front() {
        return queue[head];
    }
} fetch_queue_prev, fetch_queue_nxt;

struct Reservation_Station {
    bool busy;
    unsigned rod_id;
    op_type op;
    unsigned q1, q2, v1, v2, imm;
} res_prev[res_size], res_nxt[res_size];

struct Reorder_Buffer {
    unsigned result, ID, type;
} rob_prev[rob_size], rob_nxt[rob_size];

struct Register_File {
    unsigned q, v;
} reg_prev[reg_file_size], reg_nxt[reg_file_size];

struct fetch_to_issue_type{
    unsigned inst, pc;
} inst_fetch_to_issue;
...
void run_inst_fetch_queue(){
    ...
    tips: 考虑边界问题（满/空...）
    */
    if (fetch_queue_prev.length <= fetch_queue_size){
        fetch_queue_nxt.push(fetch(pc), pc);
    }
    if (fetch_queue_prev.length) {
        flag_fetch_queue_to_issue = true;
        inst_fetch_queue_to_issue = fetch_queue_prev.front().inst;
    }
}
